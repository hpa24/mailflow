...
## Tombstones gegen Wiederauftauchen + Konsolen-Hygiene + Copy-Button 2026-06-05 #ui

**Tombstone-Fix (Race-Condition, Commit `ae27ef6`):** Verschobene/gelöschte Mails tauchten „ziemlich häufig" oben in der Eingangsliste wieder auf und verschwanden erst beim nächsten Abruf. Ursache: Optimistic UI entfernt die Mail sofort aus State + DOM, der API-Call (IMAP-Move + PB-Patch) läuft ohne `await`; feuert in diesem Fenster `silentRefresh` (SSE `new-mail` oder 2-Min-Sync-Poll), liefert der Server die Mail noch aus → sie ist lokal unbekannt → wird als „neu" oben geprependet. Fix: Tombstone-Map (`_removedTombstones` in `inbox.js`, id → timestamp, TTL 10 Min); `silentRefresh` ignoriert tombstoned IDs. **Regel: Jeder Pfad, der Mails optimistisch aus der Liste entfernt, muss `_addTombstone(id)` setzen** (aktuell alle vier: `moveEmailsToFolder`, `deleteEmail`, `bulkDelete`, `spamEmail`) **und beim Rollback nach API-Fehler `_clearTombstone(id)` aufrufen.** Verwandt mit der Backend-Race vom 2026-06-03 (s. o. #imap-sync), aber eigenständig im Frontend.
...
## Composer-Schriftgröße: execCommand raus, kein Tab-Hänger mehr 2026-06-08 #composer #ui #performance

Die Größen-Buttons (S/M/L/XL) ließen den Tab **sekundenlang hängen** bis zum Browser-Abbruch. **Root Cause (debug-first, im Browser reproduziert):** `document.execCommand('fontSize', …)` wickelt bei großer Selektion (z. B. „Alles markieren" in einer Mail mit zitiertem Thread oder aus Word eingefügtem HTML) jeden Textknoten in `<font>`-Tags und merged sie — **O(n²)**. Gemessen: 32k Knoten → **10,7 s**, 24k Knoten (Tabellen) → **14,6 s**, also reproduzierbar über der „Seite reagiert nicht"-Schwelle.

**Fix (Commits `d15e94b` + `33706ce`):** Eigene `applyFontSize(px)` in `compose.js` — die Selektion wird **einmal** in einen `<span style="font-size:…px">` gewickelt (ein DOM-Move, O(n)) → dieselben 32k Knoten in **~30 ms** (~350–650× schneller). Buttons setzen jetzt **feste px-Werte** (`data-size` in `index.html`): **S=14 / M=18 / L=24 / XL=30** statt der unzuverlässigen Browser-Legacy-Skala 1/3/4/5.

- **Einheitlich statt verschachtelt:** `applyFontSize` entfernt vorhandene `font-size` in der Auswahl und löst leer gewordene (attributlose) `<span>`-Hüllen auf; reine font-size-Eltern-Spans, die nur noch den neuen Span enthalten, werden entwickelt. Ergebnis: **genau eine** `<span style="font-size">`-Ebene pro Bereich, eine neue Größe überschreibt innere Größen einheitlich, `<b>`/`<i>` bleiben erhalten. (Vorher stapelte jeder Klick eine weitere Ebene — sichtbar gewann nur die innerste, Quelltext blähte sich auf.)
...
## Kalender-Einladungen (.ics): Anhang-Erkennung + Event-Vorschau 2026-06-16 #kalender #ics #mime

**Symptom:** Eine Kalender-Einladung (Apple Mail zeigte `Mail-Anhang.ics`) erschien in Mailflow als „Kein Inhalt" **ohne** Anhang — in anderen Clients sichtbar.

**Root Cause (debug-first, an der echten Mail verifiziert):** Solche Invites kommen als reiner `text/calendar`-Part **ohne** `Content-Disposition: attachment` und **ohne** Dateinamen (Apple synthetisiert den Namen `Mail-Anhang.ics` selbst). Der Anhang-Filter `cd == "attachment" or filename` an drei Stellen (`mime_parser.extract_attachment_meta`/`get_attachment_payload` + BODYSTRUCTURE-Spiegel `services/imap._is_attachment_leaf`) verwarf den Part; zusätzlich war `has_attachments` aus mailparser `False`, sodass die Extraktion gar nicht erst lief.

**Fix (Commit `8473fce`):** gemeinsames Prädikat `mime_parser._is_attachment_part` — Anhang = jeder Leaf-Part, der **weder** `text/plain`/`text/html`-Body **noch** eine per Content-ID referenzierte Inline-Ressource ist (fängt `text/calendar` ohne Namen mit ab). Fallback-Dateiname mit Endung aus dem Content-Type (`anhang_1.ics`). `has_attachments` nutzt jetzt `extract_attachment_meta` (konsistent statt mailparser). `_is_attachment_leaf` identisch nachgezogen, damit der gespeicherte `part_index` auf denselben Part zeigt (sonst lädt der Download den falschen Teil).

> **Wiedereinlesen bestehender Mails:** Der Fix greift beim **IMAP-Insert neuer** Mails. Eine bereits gespeicherte Mail neu durch den Parser ziehen = PB-Record löschen, `folders.last_sync_uid` der INBOX knapp **unter** die UID setzen, Sync abwarten (`search ALL` bei `last_sync_uid==0`, sonst inkrementell ab gesetztem Wert; vorhandene Mails werden über den `message_id`-Unique-Skip übersprungen, nur die fehlende neu inserted). **Vorsicht Clobber:** ein parallel laufender Sync schreibt am Ende den alten Höchstwert via `_update_folder` zurück — `last_sync_uid` ggf. niedrig halten, bis die Ziel-UID eingelesen ist. (Mailflow ist Multi-Account: INBOX-Folder-Records existieren pro Account — am **richtigen** schrauben.)
