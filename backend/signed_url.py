...
Payload: {"p": "<path>", "e": <epoch>}  (klein gehalten — landet in URLs)
...
def sign(path: str, ttl_seconds: int = _DEFAULT_TTL) -> tuple[str, int]:
    """Gibt (token, exp_epoch) zurück."""
    ttl = max(1, min(int(ttl_seconds), _MAX_TTL))
    exp = int(time.time()) + ttl
    payload = json.dumps({"p": path, "e": exp}, separators=(",", ":")).encode("utf-8")
...
def verify(token: str, path: str) -> bool:
    """True wenn Token gültig für genau diesen path und nicht abgelaufen."""
